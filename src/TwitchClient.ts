...
import { User, APIResponse, Archive, Follows } from './NetworkModels'
...
export default class {
  ...
  async GetSingleUserID (username: string): Promise<User> {
    ...
    const response = await NetworkTasks.Get<APIResponse<User>>(
      'users',
      { login: username },
      this.clientId,
      this.accessToken
  ...
  async GetVideosForUser (userid: string) {
    await this.CheckLogin()
    const response = await NetworkTasks.Get<APIResponse<Archive>>(
      'videos',
  ...
  async GetUserFollows (userid: string): Promise<Follows[]> {
    await this.CheckLogin()
    const response = await NetworkTasks.Get<APIResponse<Follows>>(
      'users/follows',
      { from_id: userid, first: '100' },
      this.clientId,
      this.accessToken
    )
    return response.data
   ...
   */
  async Login () {
    const loginResponse = await NetworkTasks.Login(
      this.clientId,
      this.clientSecret
    )
